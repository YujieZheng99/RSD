...
def train_distill(epoch, train_loader, module_list, criterion_list, optimizer, opt):
    ...
    for idx, data in enumerate(train_loader):
        ...
        feat_s, output_s = model_s(input, is_feat=True)
        ...
        output_t, y_a, y_b, lam = model_s(input, feat_s=feat_s, y=target)
        bz = input.shape[0]
        ...
        loss_cmsd = opt.alpha * (loss_cls_s + loss_cls_t + loss_mix_ts + loss_mix_tt) + opt.beta * (loss_div + loss_div_mix)
        ...
        loss_div = criterion_kd(output_t[0:bz, :], logit_t)
        ...
        loss = 0.1 * loss_cmsd + 0.9 * loss_div
